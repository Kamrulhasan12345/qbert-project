...
int
cmp_dk (const void *a, const void *b)
{
...
void
iTile (double x, double y)
{
  double x_coords[]
      = { x, x + tile_width * cos (PI / 6), x, x - tile_width * cos (PI / 6) };
  double y_coords[]
      = { y, y - tile_width / 2, y - tile_width, y - tile_width / 2 };
  iFilledPolygon (x_coords, y_coords, 4);
}

void
iTileOutline (double x, double y)
{
  double x_coords[]
      = { x, x + tile_width * cos (PI / 6), x, x - tile_width * cos (PI / 6) };
  double y_coords[]
...
void
iSide (double x, double y)
{
  double x_coords[]
      = { x, x, x + tile_width * cos (PI / 6), x + tile_width * cos (PI / 6) };
...
void
iSideOutline (double x, double y)
{
  double x_coords[]
      = { x, x, x + tile_width * cos (PI / 6), x + tile_width * cos (PI / 6) };
  double y_coords[] = { y - tile_width, y - tile_width - tile_height,
...
iDrawEnemy (enemy_t *enemy)
{
  ...
  iSetColor (255, 0, 0);
  iFilledCircle (start_x
                     + (enemy->km.pos.z - enemy->km.pos.x) * tile_width
                           * cos (PI / 6),
...
bool
collision (player_t *player, enemy_t *enemy)
{
  if ((player->km.pos.x == enemy->km.pos.x)
      && (player->km.pos.y == enemy->km.pos.y)
...
int
iBodyMove (int x, int y, int z, body_t *km)
{
  if (!(x >= 0 && x < MAX_SIZE && y >= 0 && y < MAX_SIZE && z >= 0
        && z < MAX_SIZE))
    {
      // die and reset
...
iGetNextStep (position_t s, position_t e)
{
          ...
          if (~(int)n.x && !visited[(int)n.y][(int)n.x][(int)n.z])
            {
              ...
              prev[(int)n.y][(int)n.x][(int)n.z]
                  = { .x = queue[i].x, .y = queue[i].y, .z = queue[i].z };
            }
        }
      i++;
    }
  ...
  for (int j = 0; j < i / 2; j++)
    {
      position_t temp = path[j];
      path[j] = path[i - j - 1];
      path[i - j - 1] = temp;
    }
  if (path[0].x == s.x && path[0].y == s.y && path[0].z == s.z)
    return (position_t){ .x = path[1].x, .y = path[1].y, .z = path[1].z };
  return (position_t){ .x = path[0].x, .y = path[0].y, .z = path[0].z };
}
  ...
  if (app_state == STATE_MENU)
    {
      if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
        {
          if (mx > width / 2 - 100 && mx < width / 2 + 50 && my > 500
              && my < 530)
            {
              iGame ();
            }
