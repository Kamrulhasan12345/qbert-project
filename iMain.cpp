...
void iLoadEnemies() {
  for (int i = 0; i < world.enemy_count; i++) {
    ...
    world.enemies[i].km.pos.x = world.visible[idx][0], world.enemies[i].km.pos.y = world.visible[idx][1],
...
void iLoadLevel(int level) {
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    if (!strncmp(line, "LEVEL", 5)) {
      ...
      sscanf(line, "START %d %d %d", &gameLevel.player.km.pos.x, &gameLevel.player.km.pos.y, &gameLevel.player.km.pos.z);
      ...
      mode = 3;
    else if (!strncmp(line, "//", 2)) continue;
    else {
      switch (mode) {
      case 0: {
        int x, y, z;
        ...
        if (sscanf(line, "%d %d %d %d", &enemy_idx, &x, &y, &z) == 4) {
          if (gameLevel.enemy_count < MAX_ENEMIES) {
            gameLevel.enemies[gameLevel.enemy_count++] = {
                .km = {.jump = {0}, .pos = {.x = 1.*x, .y = 1.*y, .z = 1.*z}, .la = (look_t) (rand() % 4)},
      ...
      case 2: {
        ...
        if (sscanf(line, "%d %d %d", &r, &g, &b) == 3) {
          if (gameLevel.states_count < MAX_STATES) {
            gameLevel.states[gameLevel.states_count++] = {.r = (uint8_t)r, .g = (uint8_t)g, .b = (uint8_t)b};
          }
        }
        break;
      }
...
void iCompleteLevel() {
  ...
  // or show complete game (will set end_game to true and level_completed to false)
}
...
void iDrawQueue() {
  ...
  if (app_state == STATE_GAME) {
    ...
    for (j = 0; j < world.enemy_count; j++) {
      drawqueue[i].pos.x = world.enemies[j].km.pos.x, drawqueue[i].pos.y = world.enemies[j].km.pos.y,
...
void iEnemyStep() {
  ...
  for (int i = 0; i < world.enemy_count; i++) {
    ...
    default: {
      ...
      position_t step =
          iGetNextStep(world.enemies[i].km.jump.active ? world.enemies[i].km.jump.to : world.enemies[i].km.pos,
